...
async function waitForHealth(url,timeoutMs=15000){const started=Date.now();let lastError;while(Date.now()-started<timeoutMs){try{const res=await fetch(url);if(res.ok)return res;}catch(err){lastError=err;}await sleep(120);}throw lastError||new Error(`health check timeout: ${url}`);}
