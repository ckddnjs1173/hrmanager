...
<section class="annual-card"><div class="resource-head"><div><span class="case-kicker">연차유급휴가·미사용수당</span><h2>연차 사건</h2><p>${esc(facts.employmentStartDate || "?")} ~ ${esc(facts.employmentEndDate || "재직 중")} · 주 ${esc(facts.averageWeeklyScheduledHours ?? "?")}시간</p><span class="annual-version">${esc(legal.legalVersion?.id || "법률 버전 재확인")}</span></div><span class="annual-pill ${caseStatusPillClass(result.case?.status)}">${esc(result.case?.status || "intake")}</span></div><div class="next-action"><b>${esc(result.nextAction?.title || "다음 정보를 확인하세요.")}</b><p>${esc(result.nextAction?.description || "")}</p></div></section>
