...
- [완료 2026-09-03] "반환값 검사형"과 "throw/catch형" 중 하나를 표준 관용구로 문서화 → lib/error-utils.js 상단에 문서화(throw/catch형이 기본, {ok,errors} 반환형은 배치 검증 전용 예외). 코드 동작 변경 없음, 기존 파일 마이그레이션 안 함
...
- **[2026-09-07 발견]** advisor.css가 위 4개 세트에도 안 잡혀있던 **5번째 독립 토큰 세트**임을 확인(색 토큰이 #111827/#2563eb로 사이트 나머지의 보라 `--blue:#5b4bff` 계열과 전혀 다름, 다크 사이드바 톤). 이번 세션에서 명백한 위반(상태배지 999px 필, 카드 그림자)만 수정했고, 전체 재테마(색 토큰 교체)는 미착수 — 통합 작업 범위에 advisor.css를 5번째로 포함할 것
