...
import {
  RETIREMENT_CASE_TYPE,
  createInitialRetirementCase,
  getRetirementIntakeState,
  normalizeRetirementFacts,
} from "./retirement-intake.js";
import { buildRetirementActions, getRetirementNextAction } from "./retirement-actions.js";
import { getRetirementLegalContext } from "./retirement-rules.js";
import { buildRetirementDocuments, buildRetirementProcedures } from "./retirement-resources.js";
import { buildRetirementCaseReport } from "./retirement-report.js";
...
function mergeFacts(existingFacts = {}, patch = {}) {
  ...
  return normalizeRetirementFacts(merged);
}

function derive(facts) {
  const legal = getRetirementLegalContext(facts);
  const documents = buildRetirementDocuments(facts, legal);
  const procedures = buildRetirementProcedures(facts, legal);
  const calculations = [
    legal.averageWage ? { id: "retirement.average_wage", type: "average_wage", ...legal.averageWage } : null,
    legal.money ? { id: "retirement.benefit", type: "retirement_benefit", ...legal.money } : null,
  ].filter(Boolean);
  return { legal, documents, procedures, calculations };
}

function toCaseUpdate(existingCase, facts) {
  const generated = createInitialRetirementCase(facts);
  ...
  return {
    ...
    actions: buildRetirementActions(generated.facts),
    meta: {
      ...
      retirementMoneyStatus: derived.legal.money?.status,
    },
  };
}

function buildResult(caseRecord) {
  const intake = getRetirementIntakeState(caseRecord.facts);
  const { legal, documents, procedures } = derive(caseRecord.facts);
  const nextAction = (Array.isArray(caseRecord.actions) && caseRecord.actions[0]) || getRetirementNextAction(caseRecord.facts);
  return { case: caseRecord, intake, legal, documents, procedures, nextAction };
}

export function createRetirementCase(facts = {}) {
  const record = createInitialRetirementCase(facts);
  ...
  record.actions = buildRetirementActions(record.facts);
  record.meta = {
  ...
  return buildResult(cases.insert(record, "api:retirement-intake"));
}

export function getRetirementCase(id) {
  const found = cases.get(id);
  if (!found) return { error: "case_not_found" };
  if (found.case_type !== RETIREMENT_CASE_TYPE) return { error: "case_type_mismatch", case: found };
  return buildResult(found);
}

export function updateRetirementCase(id, factsPatch = {}) {
  const current = cases.get(id);
  if (!current) return { error: "case_not_found" };
  if (current.case_type !== RETIREMENT_CASE_TYPE) return { error: "case_type_mismatch", case: current };
  const facts = mergeFacts(current.facts, factsPatch);
  const updated = cases.update(id, toCaseUpdate(current, facts), "api:retirement-intake");
  return updated ? buildResult(updated) : { error: "case_not_found" };
}

export function renderRetirementDocument(id, templateKey, extraValues = {}) {
  const result = getRetirementCase(id);
...
export function getRetirementCaseReport(id) {
  const result = getRetirementCase(id);
  return result?.error ? result : buildRetirementCaseReport(result);
}
