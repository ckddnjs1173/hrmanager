...
async function syncSession(client, organizationId, { actorUserId = null, step = null, eventType = "SYNC", metadata = {} } = {}) {
  ...
  await client.query(
    `UPDATE business_onboarding_sessions SET
      status=$1,current_step=$2,completed_steps=$3,missing_milestones=$4,activation_signal=$5,
      completed_at=CASE WHEN $1='COMPLETED' THEN COALESCE(completed_at,$6) ELSE completed_at END,
      activated_at=CASE WHEN $5 IS NOT NULL THEN COALESCE(activated_at,$6) ELSE activated_at END,
  ...
  return withPostgresTransaction(getRuntimePostgresPool(), async (client) => {
    ...
    await client.query(
      `INSERT INTO business_profiles
       ...
       industry_code=EXCLUDED.industry_code,payday=EXCLUDED.payday,default_weekly_hours=EXCLUDED.default_weekly_hours,
       wage_system=EXCLUDED.wage_system,inclusive_wage=EXCLUDED.inclusive_wage,
       rules_of_employment_exists=EXCLUDED.rules_of_employment_exists,external_advisor_exists=EXCLUDED.external_advisor_exists,
