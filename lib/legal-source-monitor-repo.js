...
async function governanceEvent(client, { actor, eventType, metadata = {}, createdAt = nowIso() }) {
  await client.query(
    `INSERT INTO legal_governance_events
     ...
     VALUES ($1,NULL,NULL,$2,$3,NULL,NULL,$4,$5)`,
    [id("lge"), actor, eventType, JSON.stringify(metadata || {}), createdAt],
  );
}
  ...
  try {
    return await withPostgresTransaction(getRuntimePostgresPool(), async (client) => {
      ...
      await governanceEvent(client, {
        ...
        metadata: { watchId, canonicalSourceId: source.id, sourceType, officialUrl: source.url },
        createdAt: now,
      });
...
export async function setLegalSourceWatchEnabled({ watchId, enabled, actor } = {}) {
  ...
  return withPostgresTransaction(getRuntimePostgresPool(), async (client) => {
    ...
    await governanceEvent(client, {
      ...
      metadata: { watchId, fromEnabled: previous, toEnabled: enabled },
      createdAt: now,
    });
    return mapWatch(result.rows[0]);
  });
...
async function markRunFailed({ runId, watchId, error, now, triggeredBy }) {
  ...
  await withPostgresTransaction(getRuntimePostgresPool(), async (client) => {
    ...
    await governanceEvent(client, {
      ...
      metadata: { watchId, runId, errorCode },
  ...
  try {
    return await withPostgresTransaction(getRuntimePostgresPool(), async (client) => {
      ...
      await governanceEvent(client, {
        ...
        metadata: { watchId, runId, previousContentHash: previousHash, currentContentHash: fetched.contentHash, candidateId },
        createdAt: finishedAt,
      });
