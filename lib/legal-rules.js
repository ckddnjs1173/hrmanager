...
const SOURCES = Object.freeze({
  ...
  LABOR_STANDARDS_56: Object.freeze({
    id: "source.lsa.article56",
    authority: "국가법령정보센터",
    title: "근로기준법 제56조 연장·야간 및 휴일 근로",
    article: "근로기준법 제56조",
    url: "https://law.go.kr/lsLinkCommonInfo.do?chrClsCd=010202&lsJoLnkSeq=1025590459",
    verifiedAt: VERIFIED_AT,
  }),
  LABOR_STANDARDS_DECREE_APPENDIX_1: Object.freeze({
    id: "source.lsa_decree.appendix1",
    authority: "국가법령정보센터",
    title: "근로기준법 시행령 별표 1 상시 4명 이하 사업장 적용 규정",
...
export const WAGE_LEGAL_BASELINE = Object.freeze({
  ...
  premiumCurrent: Object.freeze({
    id: "wage.premium.current_baseline",
    category: "wage_premium",
    overtimePremiumRate: 0.5,
    nightPremiumRate: 0.5,
    holidayWithin8PremiumRate: 0.5,
    holidayOver8PremiumRate: 1,
    source: SOURCES.LABOR_STANDARDS_56,
    workplaceScopeSource: SOURCES.LABOR_STANDARDS_DECREE_APPENDIX_1,
  }),
});
...
export function getWageLegalContext(facts = {}, { asOfDate = null } = {}) {
  ...
  const hasPremiumIssue = [facts.overtimeWork, facts.nightWork, facts.holidayWork].some((value) => value === true);
  const sources = [];
  const warnings = [];
  ...
  if (hasPremiumIssue) {
    sources.push(WAGE_LEGAL_BASELINE.premiumCurrent.source);
    sources.push(WAGE_LEGAL_BASELINE.premiumCurrent.workplaceScopeSource);
  }
  ...
  return {
    ...
    delayInterestRule:
      employmentEnded && facts.employmentEndDate >= WAGE_LEGAL_BASELINE.delayInterestCurrent.effectiveFrom
        ...
        : null,
    premiumRule: hasPremiumIssue ? WAGE_LEGAL_BASELINE.premiumCurrent : null,
