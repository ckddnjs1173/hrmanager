...
export function getNextWageQuestions(input = {}, limit = 3) {
  ...
  const stepMissing = missing.filter(
    (key) => QUESTION_DEFINITIONS[key]?.step === step
  );

  const priority = getQuestionPriority(step);

  const ordered = [
    ...priority.filter((key) => stepMissing.includes(key)),
    ...stepMissing.filter((key) => !priority.includes(key)),
  ];
