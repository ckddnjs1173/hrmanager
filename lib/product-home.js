...
import { resolvePublicSiteOrigin, rewritePublicDocumentMetadata } from "./public-seo.js";
...
export function prepareProductHomeHtml(html = "", { siteOrigin = null } = {}) {
  ...
  const deploymentAware = rewritePublicDocumentMetadata(html, { siteOrigin, pathname: "/" });
  const localFontOnly = injectBodyTag(injectHeadTag(stripLegacyExternalFontLinks(deploymentAware), '<link rel="stylesheet" href="/global-navigation.css">'), '<script src="/global-navigation.js"></script>');
...
export function createProductHomeHandler(rootDir, { env = process.env } = {}) {
  ...
  return (_req, res, next) => {
    fs.readFile(indexPath, "utf8", (err, html) => {
      ...
      res.send(prepareProductHomeHtml(html, { siteOrigin }));
    });
  };
}

export function createProductEntryHandler(rootDir, fileName, {env = process.env} = {}) {
  const siteOrigin = resolvePublicSiteOrigin(env);
  return (_req, res, next) => {
    fs.readFile(path.join(rootDir, fileName), 'utf8', (error, html) => {
      if(error) return next(error);
      res.setHeader('Content-Type', 'text/html; charset=utf-8');
      res.setHeader('Cache-Control', 'no-cache, must-revalidate');
      const prepared = rewritePublicDocumentMetadata(html, {siteOrigin, pathname:`/${fileName}`});
      // Entry pages include a deferred shell themselves; avoid duplicate script injection.
      res.send(prepared);
    });
  };
}
