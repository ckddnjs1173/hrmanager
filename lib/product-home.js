...
export function injectProductPrivacyScript(html = "") {
  const source = String(html || "");
  if (source.includes(PRODUCT_PRIVACY_SCRIPT)) return source;
  if (/<\/body>/i.test(source)) {
    return source.replace(/<\/body>/i, `${PRODUCT_PRIVACY_SCRIPT}\n</body>`);
  }
  return `${source}\n${PRODUCT_PRIVACY_SCRIPT}`;
}
...
export function prepareProductHomeHtml(html = "") {
  ...
  return injectProductHomeScript(
