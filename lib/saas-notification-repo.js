...
async function cancelStalePending(client, now) {
  ...
  const structurallyStale = await client.query(
    ...
    [timestamp, ACTIVE_ACTION_STATUSES, DEADLINE_NOTIFICATION_RECIPIENT_ROLES]
  );

  const pending = await client.query(
    `SELECT n.id,n.notification_key,a.due_at
     FROM compliance_notification_outbox n
     JOIN compliance_actions a ON a.id=n.source_id AND a.organization_id=n.organization_id
     WHERE n.status='PENDING' AND n.channel='IN_APP' AND n.source_type='COMPLIANCE_ACTION'
       AND a.status = ANY($1::text[]) AND a.due_at IS NOT NULL`,
    [ACTIVE_ACTION_STATUSES]
  );
  let milestoneStale = 0;
  for (const row of pending.rows) {
    const currentKey = deadlineNotificationKey({ dueAt: row.due_at, now });
    if (currentKey === row.notification_key) continue;
    const cancelled = await client.query(
      `UPDATE compliance_notification_outbox SET status='CANCELLED',cancelled_at=$1
       WHERE id=$2 AND status='PENDING'`,
      [timestamp, row.id]
    );
    milestoneStale += cancelled.rowCount || 0;
  }
  return (structurallyStale.rowCount || 0) + milestoneStale;
}
    ...
    for (const row of result.rows) {
      ...
      const inserted = await client.query(
        `INSERT INTO compliance_notification_outbox
         ...
         WHERE compliance_notification_outbox.status='CANCELLED'`,
        [
          outboxId,
          row.organization_id,
