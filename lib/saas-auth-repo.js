...
export async function issueMagicChallenge({ email, ttlMinutes = 15, ipHash = null, requestId = null } = {}) {
  ...
  if (!emailNormalized || !emailNormalized.includes("@")) throw new Error("invalid_email");
...
export async function consumeMagicChallenge({ token, sessionTtlDays = 30, ipHash = null, userAgent = "", requestId = null } = {}) {
  ...
  return withPostgresTransaction(getRuntimePostgresPool(), async (client) => {
    ...
    if (!challenge) {
      await recordSecurityEvent({ event: "auth.magic.verify", result: "DENIED", requestId, ipHash, metadata: { reason: "not_found" } }, client);
      throw new Error("magic_token_invalid");
    }
    if (challenge.consumed_at) throw new Error("magic_token_consumed");
    if (new Date(challenge.expires_at).getTime() <= Date.now()) {
      await client.query("UPDATE auth_challenges SET consumed_at=$1 WHERE id=$2", [createdAt, challenge.id]);
      throw new Error("magic_token_expired");
    }

    await client.query("UPDATE auth_challenges SET consumed_at=$1 WHERE id=$2", [createdAt, challenge.id]);
    let userResult = await client.query("SELECT * FROM users WHERE email_normalized=$1 FOR UPDATE", [challenge.email_normalized]);
    let user = userResult.rows[0];
    if (user?.status === "locked" || user?.status === "deleted") throw new Error("user_unavailable");
