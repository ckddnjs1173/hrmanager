...
import {
  DISMISSAL_CASE_TYPE,
  createInitialDismissalCase,
  getDismissalIntakeState,
  getDismissalQuestions,
  normalizeDismissalFacts,
} from "./dismissal-intake.js";
import { buildDismissalActions, getDismissalNextAction } from "./dismissal-actions.js";
import { getDismissalLegalContext } from "./dismissal-rules.js";
import { buildDismissalDocuments, buildDismissalProcedures } from "./dismissal-resources.js";
import { buildDismissalCaseReport } from "./dismissal-report.js";
...
function mergeFacts(existingFacts = {}, patch = {}) {
  ...
  return normalizeDismissalFacts(merged);
}

function derive(facts) {
  const legal = getDismissalLegalContext(facts);
  const documents = buildDismissalDocuments(facts, legal);
  const procedures = buildDismissalProcedures(facts, legal);
  const calculations = legal.noticeAllowance
    ? [{
        id: "dismissal.notice_allowance",
        type: "dismissal_notice_allowance",
        ...legal.noticeAllowance,
        sourceId: "source.lsa.article26",
      }]
    : [];
...
function toCaseUpdate(existingCase, facts) {
  const generated = createInitialDismissalCase(facts);
  ...
  return {
    ...
    actions: buildDismissalActions(generated.facts),
    meta: {
...
function buildResult(caseRecord) {
  const intake = getDismissalIntakeState(caseRecord.facts);
  const { legal, documents, procedures } = derive(caseRecord.facts);
  const nextAction =
    (Array.isArray(caseRecord.actions) && caseRecord.actions[0]) ||
    getDismissalNextAction(caseRecord.facts);

  return {
    case: caseRecord,
    intake,
    questions: getDismissalQuestions(caseRecord.facts, 3),
    legal,
    documents,
    procedures,
    nextAction,
...
export function createDismissalCase(facts = {}) {
  const record = createInitialDismissalCase(facts);
  ...
  record.actions = buildDismissalActions(record.facts);
  record.meta = {
    ...(isObject(record.meta) ? record.meta : {}),
    legalReferenceDate: derived.legal.referenceDate,
    legalVerifiedAt: derived.legal.verifiedAt,
  };
  const created = cases.insert(record, "api:dismissal-intake");
  return buildResult(created);
}

export function getDismissalCase(id) {
  const found = cases.get(id);
  if (!found) return { error: "case_not_found" };
  if (found.case_type !== DISMISSAL_CASE_TYPE) return { error: "case_type_mismatch", case: found };
  return buildResult(found);
}

export function updateDismissalCase(id, factsPatch = {}) {
  const current = cases.get(id);
  if (!current) return { error: "case_not_found" };
  if (current.case_type !== DISMISSAL_CASE_TYPE) return { error: "case_type_mismatch", case: current };

  const facts = mergeFacts(current.facts, factsPatch);
  const updated = cases.update(id, toCaseUpdate(current, facts), "api:dismissal-intake");
  if (!updated) return { error: "case_not_found" };
  return buildResult(updated);
}

export function renderDismissalDocument(id, templateKey, extraValues = {}) {
  const result = getDismissalCase(id);
  ...
  return { templateKey, values, document: rendered, caseId: id };
}

export function getDismissalCaseReport(id) {
  const result = getDismissalCase(id);
  if (result?.error) return result;
  return buildDismissalCaseReport(result);
}
