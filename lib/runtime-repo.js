import {rank as rankDirectory} from "../directory-policy.js";
...
export const nomusa = {
  ...
  async publicList({ region = "" } = {}) {
    ...
    const args = [];
    let where = "WHERE opted_out=FALSE";
    if (region) {
      // LIKE is deliberately case-sensitive, matching SQLite's literal includes filter.
      // Escape wildcard characters so a region such as '%' cannot widen the result set.
      args.push(`%${region.replace(/[!%_]/g, "!$&")}%`);
      where += " AND loc LIKE $1 ESCAPE '!'";
    }
    const r = await pg().query(`SELECT doc,featured FROM nomusa ${where}`, args);
    return rankDirectory(r.rows.map(row => ({...(row.doc || {}), featured:!!row.featured})), {region});
  },
  async adminList() {
