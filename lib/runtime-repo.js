...
export const nomusa = {
  ...
  async publicList({ region = "" } = {}) {
    ...
    const r = await pg().query("SELECT doc,featured FROM nomusa WHERE opted_out=FALSE");
    const list = r.rows.map((row) => ({ ...(row.doc || {}), featured: !!row.featured }))
      .filter(item => !region || (item.loc || "").includes(region));
    return globalThis.INSAYA_DIRECTORY.rank(list, { region });
  },
  async adminList() {
