...
const NOT_FOUND_CODES = new Set([
  ...
  "business_case_organization_not_active",
  "external_advisor_grant_not_found",
  "external_advisor_resource_not_found",
  "external_advisor_organization_not_active",
...
const FORBIDDEN_CODES = new Set([
  "business_case_actor_not_active",
  ...
  "external_advisor_creator_not_active",
  "external_advisor_user_not_active",
  "external_advisor_revoker_not_active",
  "csrf_invalid",
]);
...
export function createSaasAdvisorCollaborationRouter({
  ...
  const getService = () => {
    if (!collaborationService) collaborationService = createExternalAdvisorCollaborationService();
  ...
  router.post(
    ...
    (req, res) => send(res, async () => ({
      businessCase: await getService().createBusinessCase({
  ...
  router.get(
    ...
    requireAuth,
    (req, res) => send(res, async () => ({
      businessCases: await getService().listBusinessCases({
        organizationId: req.params.organizationId,
        actorUserId: req.saasSession.userId,
  ...
  router.patch(
    ...
    rateLimit({ max: 60 }),
    (req, res) => send(res, async () => ({
      businessCase: await getService().transitionBusinessCase({
  ...
  router.post(
    ...
    rateLimit({ max: 30 }),
    (req, res) => send(res, async () => ({
      shareGrant: await getService().issueExternalAdvisorShareGrant({
        organizationId: req.params.organizationId,
        caseId: req.params.caseId,
  ...
  router.get(
    ...
    requireAuth,
    (req, res) => send(res, async () => ({
      shareGrants: await getService().listOrganizationShareGrants({
        organizationId: req.params.organizationId,
        actorUserId: req.saasSession.userId,
        limit: req.query.limit || 100,
      }),
  ...
  router.post(
    ...
    rateLimit({ max: 60 }),
    (req, res) => send(res, async () => ({
      shareGrant: await getService().revokeExternalAdvisorShareGrant({
  ...
  router.get(
    ...
    requireAuth,
    (req, res) => send(res, async () => ({
      shareGrants: await getService().listAdvisorShareGrants({
  ...
  router.get(
    "/advisor/share-grants/:grantId/case",
    requireAuth,
    (req, res) => send(res, async () => getService().getSharedBusinessCaseForAdvisor({
      grantId: req.params.grantId,
      actorUserId: req.saasSession.userId,
    })),
  );
  ...
  router.post(
    ...
    rateLimit({ max: 60 }),
    (req, res) => send(res, async () => ({
      shareGrant: await getService().acceptExternalAdvisorShareGrant({
        grantId: req.params.grantId,
        actorUserId: req.saasSession.userId,
      }),
    })),
