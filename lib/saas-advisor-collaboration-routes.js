...
const BAD_REQUEST_PREFIXES = [
  ...
  "business_case_document_content_", "business_case_document_review_",
...
const NOT_FOUND_CODES = new Set([
  ...
  "business_case_document_content_not_found",
]);
...
const SERVICE_UNAVAILABLE_CODES = new Set(["business_case_document_storage_unavailable"]);

const errorStatus = createErrorStatusResolver([
  { codes: NOT_FOUND_CODES, status: 404 },
  { codes: FORBIDDEN_CODES, status: 403 },
  { codes: CONFLICT_CODES, status: 409 },
  { codes: SERVICE_UNAVAILABLE_CODES, status: 503 },
  { codes: BAD_REQUEST_CODES, prefixes: BAD_REQUEST_PREFIXES, status: 400 },
]);
...
function contentDisposition(fileName) {
  const encoded = encodeURIComponent(String(fileName || "document")).replace(/['()]/g, escape);
...
export function createSaasAdvisorCollaborationRouter({
  ...
  const documentRawBody = express.raw({ type: () => true, limit: "10mb" });
  ...
  const getDocumentService = () => caseDocumentService || (caseDocumentService = createBusinessCaseDocumentService({ env }));
  ...
  function sendBinary(res, fn) {
    return Promise.resolve().then(fn).then((file) => {
      res.status(200);
  ...
  // Metadata-only reservations remain available for internal/future object-store adapters. The user-facing
  // review transition below requires encrypted content, so a metadata-only version cannot be submitted.
  ...
  router.post(
    "/business-case-documents/:documentId/content",
    requireAuth,
    requireCsrf,
    rateLimit({ max: 20 }),
    documentRawBody,
    (req, res) => send(res, async () => ({ version: await getDocumentService().uploadBusinessDocumentVersion({
      documentId: req.params.documentId,
      actorUserId: req.saasSession.userId,
      fileName: req.query.fileName,
      mimeType: String(req.get("content-type") || "").split(";", 1)[0].trim().toLowerCase(),
      bytes: Buffer.isBuffer(req.body) ? req.body : null,
    }) }), { created: true }),
  );
  router.get("/business-case-document-versions/:versionId/download", requireAuth, rateLimit({ max: 60 }), (req, res) => sendBinary(res, () => getDocumentService().downloadBusinessDocumentVersion({ versionId: req.params.versionId, actorUserId: req.saasSession.userId })));
  router.get("/business-case-documents/:documentId/access-events", requireAuth, (req, res) => send(res, async () => ({ accessEvents: await getDocumentService().listBusinessDocumentAccessEvents({ documentId: req.params.documentId, actorUserId: req.saasSession.userId, limit: req.query.limit || 200 }) })));
  router.post("/business-case-documents/:documentId/submit-review", requireAuth, requireCsrf, rateLimit({ max: 30 }), (req, res) => send(res, async () => ({ document: await getDocumentService().submitStoredBusinessDocumentForReview({ documentId: req.params.documentId, actorUserId: req.saasSession.userId }) })));
  ...
  router.get("/advisor/share-grants/:grantId/document-versions/:versionId/download", requireAuth, rateLimit({ max: 60 }), (req, res) => sendBinary(res, () => getDocumentService().downloadAdvisorDocumentVersion({ grantId: req.params.grantId, versionId: req.params.versionId, actorUserId: req.saasSession.userId })));
  ...
  router.post("/advisor/invitations/accept", requireAuth, requireCsrf, rateLimit({ max: 30 }), (req, res) => send(res, async () => getService().acceptExternalAdvisorInvitation({ rawToken: req.body?.token, actorUserId: req.saasSession.userId, actorEmail: req.saasSession.userId ? req.saasSession.user?.email : null })));
