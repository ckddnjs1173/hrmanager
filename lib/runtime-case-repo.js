...
function jsonFallback(key){return key==="facts"||key==="meta"?{}:[];}
function pgValue(key,value){return JSON_FIELDS.includes(key)?JSON.stringify(value??jsonFallback(key)):value;}
...
function mapCase(row){
  ...
  for(const key of JSON_FIELDS){
    if(typeof out[key]==="string"){try{out[key]=JSON.parse(out[key]);}catch{out[key]=jsonFallback(key);}}
    else if(out[key]==null) out[key]=jsonFallback(key);
  }
  return out;
}
...
export const cases={
  async insert(rec={},actor="user"){
    ...
    const columns=Object.keys(row);const values=columns.map((key)=>pgValue(key,row[key]));const placeholders=values.map((_,i)=>`$${i+1}`).join(",");
