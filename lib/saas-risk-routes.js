...
import {
  ...
  setComplianceActionDueDate,
} from "./saas-calendar-repo.js";
import {
  getUnreadInAppNotificationCount,
  listInAppNotifications,
  markInAppNotificationRead,
} from "./saas-notification-repo.js";
...
function errorStatus(error) {
  ...
  if (["compliance_scope_not_found", "compliance_action_not_found", "notification_not_found"].includes(code)) return 404;
  if (["permission_denied", "csrf_invalid"].includes(code)) return 403;
  if (code.startsWith("compliance_action_") || code.startsWith("risk_") || code.startsWith("calendar_") || code.startsWith("notification_")) return 400;
  return 500;
}
...
export function createSaasRiskRouter({ env = process.env, rateLimit } = {}) {
  ...
  router.patch(
    "/organizations/:organizationId/actions/:actionId/due-date",
    requireAuth,
    requireCsrf,
    requireOrg("compliance.manage"),
    async (req, res) => {
      try {
        const result = await setComplianceActionDueDate({
          organizationId: req.params.organizationId,
          actionId: req.params.actionId,
          actorUserId: req.saasSession.userId,
  ...
  router.get(
    "/organizations/:organizationId/compliance-calendar",
    requireAuth,
    requireOrg("compliance.manage"),
    async (req, res) => {
      try {
        return res.json(await getComplianceCalendar({
          organizationId: req.params.organizationId,
    ...
    async (req, res) => {
      try {
        ...
        const [notifications, unreadCount] = await Promise.all([
          listInAppNotifications({
            organizationId: req.params.organizationId,
            recipientUserId: req.saasSession.userId,
            unreadOnly,
