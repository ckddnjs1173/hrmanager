...
export function createSecureSummaryRouter({ sessionSecret }) {
  ...
  router.get("/r/:token", (req, res) => {
    ...
    if (!record) {
      return res.status(404).send(renderBrandedPage(
        "링크를 찾을 수 없습니다",
        renderStateMarkup("🔍", "유효하지 않은 링크예요", "주소가 정확한지 확인하거나, 운영자에게 링크 재발급을 요청해 주세요."),
      ));
    }

    if (record.expires && new Date(record.expires) < new Date()) {
      return res.status(410).send(renderBrandedPage(
        "만료된 링크",
        renderStateMarkup("⏰", "만료된 열람 링크예요", "보안을 위해 링크는 발급 후 일정 기간만 유효합니다. 운영자에게 재발급을 요청해 주세요."),
      ));
    }
