...
// 진단용: 최소 Gemini 호출로 상태/에러를 확인(키 미노출). model 지정 가능. 문제 해결 후 제거.
export async function geminiSelftest(model) {
  ...
  try {
    ...
    return { model: m, status: res.status, ok: res.ok, body: t.slice(0, 240) };
  } catch (e) {
    return { model: m, error: String(e?.message || e).slice(0, 240) };
  }
}
