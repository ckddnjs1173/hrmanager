...
import { semanticChecksumRows } from "./portable-semantics.js";
...
export function buildPortableExport({ readRows, exportedAt = new Date().toISOString() } = {}) {
  ...
  for (const table of LEGACY_CORE_TABLES) {
    ...
    tables[table] = {
      ...
      semanticSha256: semanticChecksumRows(table, rows),
      rows,
    };
  }
...
export function validatePortableExport(payload) {
  ...
  for (const table of LEGACY_CORE_TABLES) {
    ...
    if (entry.semanticSha256 && entry.semanticSha256 !== semanticChecksumRows(table, entry.rows)) {
      errors.push(`semantic_checksum_mismatch:${table}`);
    }
  }
