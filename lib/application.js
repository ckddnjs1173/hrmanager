...
import { createProductHomeHandler, createProductEntryHandler } from "./product-home.js";
...
export function createApplication({ rootDir, env = process.env, warn = console.warn } = {}) {
  ...
  for (const entry of ["worker", "employer", "tools"]) {
    app.get([`/${entry}`, `/${entry}.html`], createProductEntryHandler(rootDir, `${entry}.html`, {env}));
  }
  ...
  app.get("/business-login.html", createUserFacingPageHandler(rootDir, "business-login.html", { styles: saasStyles, globalNavigation: true }));
