...
export function createApplication({ rootDir, env = process.env, warn = console.warn } = {}) {
  ...
  // Liveness stays minimal; readiness is a canonical, non-rate-limited operational endpoint.
  ...
  app.use(createApplicationErrorHandler({
    warn,
    renderHtml: ({ requestId }) => renderBrandedPage(
      "요청을 처리하지 못했습니다",
      renderStateMarkup(
        "⚠️",
        "요청을 처리하지 못했어요",
        `잠시 후 다시 시도해 주세요.<br><small>요청 ID ${requestId}</small>`,
      ),
    ),
  }));
