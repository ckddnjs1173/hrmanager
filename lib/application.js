...
export function createApplication({ rootDir, env = process.env, warn = console.warn } = {}) {
  ...
  // Configured production email delivery gets first chance at magic-link and Advisor-invitation routes.
  // When not configured it calls next(), preserving the existing dev debug-token/fail-closed behavior.
