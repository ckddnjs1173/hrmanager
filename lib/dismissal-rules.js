...
function hasCompletedCalendarMonths(start, end, months) {
  const anniversary = addCalendarMonthsClamped(start, months);
  return !!anniversary && typeof end === "string" && end >= anniversary;
}
...
function noticeAllowanceAssessment(facts) {
  ...
  const hasServiceDates = !!parseIsoDate(f.employmentStartDate) && !!parseIsoDate(f.effectiveDate);
  const underThreeMonths = hasServiceDates && !hasCompletedCalendarMonths(f.employmentStartDate, f.effectiveDate, 3);
  ...
  if (!hasServiceDates || noticeDays === null || f.noticePayPaid === null) {
    return {
      status: "needs_input",
      serviceDays,
      noticeDays,
