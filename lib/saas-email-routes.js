...
import {
  resolveAdvisorInvitationForDelivery,
  resolveOrganizationInvitationForDelivery,
} from "./saas-invitation-delivery-repo.js";
...
const errorStatus = createErrorStatusResolver([
  { codes: ["invalid_email", "invitation_role_invalid", "invitation_invalid", "external_advisor_invitation_not_found", "external_advisor_permission_invalid", "external_advisor_permissions_required"], status: 400 },
  { codes: ["authentication_required"], status: 401 },
  { codes: ["csrf_invalid", "permission_denied", "external_advisor_management_role_required"], status: 403 },
  { codes: ["organization_not_found", "business_case_not_found", "external_advisor_cross_tenant_case_forbidden"], status: 404 },
  { codes: ["external_advisor_invitation_pending_duplicate", "external_advisor_business_case_not_shareable"], status: 409 },
  { codes: ["saas_email_delivery_not_configured", "saas_email_delivery_failed"], status: 503 },
]);
...
export function createSaasEmailRouter({ env = process.env, rateLimit, delivery = null, collaborationService = null } = {}) {
  ...
  async function sendBoundMagicLink({ req, to, returnTo, advisorInviteToken = "", organizationInviteToken = "" }) {
    const config = req.saasEmailConfig;
  ...
  router.post("/auth/magic-link", requireConfiguredEmailRoute, authRateLimit, async (req, res) => {
    try {
      ...
      const advisorInviteToken = returnTo === "/advisor.html" ? String(req.body?.advisorInviteToken || "").trim() : "";
      const challenge = await sendBoundMagicLink({ req, to: req.body?.email, returnTo, advisorInviteToken });
      return res.status(202).json({ ok: true, expiresAt: challenge.expiresAt, deliveryMode: "EMAIL", returnTo });
    } catch (error) {
      return res.status(errorStatus(error)).json({ error: errorCode(error) });
    }
  });

  router.post("/invitations/magic-link", requireConfiguredEmailRoute, authRateLimit, async (req, res) => {
    try {
      const rawToken = String(req.body?.token || "").trim();
      const invitation = await resolveOrganizationInvitationForDelivery(rawToken);
      const challenge = await sendBoundMagicLink({
        req,
        to: invitation.email,
        returnTo: "/business.html",
        organizationInviteToken: rawToken,
      });
  ...
  router.post("/advisor/invitations/magic-link", requireConfiguredEmailRoute, authRateLimit, async (req, res) => {
    try {
      const rawToken = String(req.body?.token || "").trim();
      const invitation = await resolveAdvisorInvitationForDelivery(rawToken);
      const challenge = await sendBoundMagicLink({
        req,
        to: invitation.email,
        returnTo: "/advisor.html",
        advisorInviteToken: rawToken,
      });
      return res.status(202).json({ ok: true, expiresAt: challenge.expiresAt, deliveryMode: "EMAIL" });
    } catch (error) {
      return res.status(errorStatus(error)).json({ error: errorCode(error) });
