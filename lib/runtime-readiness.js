...
export function evaluatePersistenceStatus({ env = process.env, storageInfo = null } = {}) {
  const runtime = storageInfo || describeStorageRuntime(env);
  ...
  const sqlitePathConfigured = runtime.primary === "sqlite" && Boolean(env.DB_PATH) && env.DB_PATH !== ":memory:";
...
export async function getRuntimeReadiness({ env = process.env } = {}) {
  ...
  const persistence = evaluatePersistenceStatus({ env, storageInfo: runtime });
