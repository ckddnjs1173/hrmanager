...
import { evaluateProductionDeploymentConfig } from "./production-deployment-contract.js";
...
function safeDeploymentStatus(env) {
  const status = evaluateProductionDeploymentConfig(env);
  return {
    ok: status.ok,
    production: status.production,
    saasEnabled: status.saasEnabled,
    persistentRequired: status.persistentRequired,
...
export async function getRuntimeReadiness({ env = process.env } = {}) {
  const deployment = safeDeploymentStatus(env);
  let runtime;
  try { runtime = describeStorageRuntime(env); }
    ...
    return {
      ...
      deployment,
      warnings: ["storage_runtime_invalid", ...deployment.warnings, ...deployment.errors.map((item) => `deployment:${item}`)],
    };
  }
  ...
  const ready = database.ok && legal.ok && cases.ok && persistence.requirementSatisfied && deployment.ok;
  ...
  for (const warning of deployment.warnings) if (!warnings.includes(warning)) warnings.push(warning);
  for (const error of deployment.errors) warnings.push(`deployment:${error}`);
