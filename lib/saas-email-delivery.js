...
export function createSaasEmailDelivery({ env = process.env, fetchImpl = globalThis.fetch } = {}) {
  ...
  async function sendMagicLink({ to, rawToken, expiresAt, challengeId, returnTo = "/business.html", advisorInviteToken = "", organizationInviteToken = "" }) {
    const hash = new URLSearchParams();
    hash.set("magic", String(rawToken || ""));
    hash.set("return", safeReturnTo(returnTo));
    if (advisorInviteToken) hash.set("invite", String(advisorInviteToken));
    if (organizationInviteToken) hash.set("orgInvite", String(organizationInviteToken));
    const link = `${config.siteUrl}/business-login.html#${hash.toString()}`;
    const advisor = safeReturnTo(returnTo) === "/advisor.html";
    return send({
      to,
      subject: advisor ? "[인사야] Advisor 로그인 링크" : "[인사야] Business 로그인 링크",
      idempotencyKey: `magic:${challengeId}`,
      html: mailShell({
        eyebrow: advisor ? "INSAYA ADVISOR" : "INSAYA BUSINESS",
        title: advisor ? "외부 자문 포털 로그인" : "회사 계정 로그인",
        body: advisor
          ? "아래 버튼을 눌러 초대받은 이메일 계정으로 로그인하세요. 로그인 후 원래 검토 초대로 안전하게 돌아갑니다."
          : "아래 버튼을 눌러 인사야 Business에 로그인하세요. 이 링크는 한 번만 사용할 수 있습니다.",
        buttonLabel: advisor ? "Advisor 로그인" : "인사야 Business 로그인",
        link,
  ...
  async function sendOrganizationInvitation({ to, rawToken, invitationId, roleKey, organizationName, expiresAt }) {
    const link = `${config.siteUrl}/business-login.html#orgInvite=${encodeURIComponent(rawToken)}`;
    ...
    return send({
      ...
      html: mailShell({
        ...
        body: `인사야 Business의 <b>${escapeHtml(roleLabel)}</b> 권한으로 초대되었습니다. 아래 버튼을 누르면 초대받은 이메일로 1회용 로그인 링크를 보내드립니다.`,
