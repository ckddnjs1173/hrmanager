...
import { bookings, leads, events, EVENT_TYPES, feedback } from "./repo.js";
...
export function createPublicOperationRouter({ rateLimit, clean }) {
  ...
  router.post("/privacy/delete", rateLimit({ max: 10 }), (req, res) => {
    ...
    if (contact) {
      ...
      return res.status(202).json(queuePrivacyDeletion(contact));
    }
    // Booking tokens authorize the expert-facing summary link only. They must never double as
    // destructive privacy capabilities because those links can be shared with operators/experts.
