...
import { deleteByActivePrivacyToken, queuePrivacyDeletion } from "./privacy-operations.js";
...
export function createPublicOperationRouter({ rateLimit, clean }) {
  ...
  router.post("/privacy/delete", rateLimit({ max: 10 }), (req, res) => {
    ...
    const contact = clean(req.body?.contact);
    if (token) return res.json({ ok: true, deleted: deleteByActivePrivacyToken(token) });
    if (contact) {
