...
function renderOrganizations() {
  ...
  for(const item of state.organizations){const option=document.createElement("option");option.value=item.organization.id;option.textContent=item.organization.displayName||item.organization.legalName||item.organization.id;picker.append(option);}
...
$("employee-form").addEventListener("submit",async(e)=>{e.preventDefault();const form=e.currentTarget;const fd=new FormData(form);try{await api(`/organizations/${state.orgId}/employees`,{method:"POST",body:{displayName:fd.get("displayName"),employeeNumber:fd.get("employeeNumber")||undefined,hireDate:fd.get("hireDate"),workplaceId:fd.get("workplaceId")||undefined,weeklyContractHours:cleanNumber(fd.get("weeklyContractHours")),wageType:fd.get("wageType")||undefined,baseWage:cleanNumber(fd.get("baseWage"))}});form.reset();form.hidden=true;await loadOrg(state.orgId);flash("직원을 추가했습니다.");}catch(error){flash(error.message,"error");}});
$("profile-form").addEventListener("submit",async(e)=>{e.preventDefault();const form=e.currentTarget;const fd=new FormData(form);const profile={};for(const key of ["industryCode","wageSystem"]){if(fd.get(key))profile[key]=fd.get(key);}for(const key of ["payday","defaultWeeklyHours"]){if(fd.get(key)!=="")profile[key]=cleanNumber(fd.get(key));}try{await api(`/organizations/${state.orgId}/business-profile`,{method:"PUT",body:{profile}});await loadOrg(state.orgId);flash("회사정보를 저장했습니다.");}catch(error){flash(error.message,"error");}});
$("workplace-form").addEventListener("submit",async(e)=>{e.preventDefault();const form=e.currentTarget;const fd=new FormData(form);try{await api(`/organizations/${state.orgId}/workplaces`,{method:"POST",body:{name:fd.get("name"),openedAt:fd.get("openedAt")||undefined}});form.reset();await loadOrg(state.orgId);flash("사업장을 추가했습니다.");}catch(error){flash(error.message,"error");}});
$("scope-form").addEventListener("submit",async(e)=>{e.preventDefault();const form=e.currentTarget;const fd=new FormData(form);try{await api(`/organizations/${state.orgId}/compliance-scopes`,{method:"POST",body:{name:fd.get("name"),status:fd.get("status"),basis:fd.get("basis")||"",workplaceIds:state.workplaces.map(w=>w.id)}});form.reset();await loadOrg(state.orgId);flash("법률 적용범위를 저장했습니다.");}catch(error){flash(error.message,"error");}});
